...
```
1. bundie_analyze_wallet("0xUSER")
   → Risk profile: Moderate, $5,000 idle USDC detected

2. bundie_get_recommendation("0xUSER", tokens=["USDC"])
   → Bundle: 40% Morpho (Base, 9.1%), 35% Fluid (Scroll, 8.5%), 25% Moonwell (Optimism, 7.8%)

3. bundie_deposit("0xUSER", asset="USDC", amount="5000")
   → Funds in Bundie vault

4. bundie_strategy_deposit("0xUSER", protocolId="morpho-uuid", amount="2000")
   bundie_strategy_deposit("0xUSER", protocolId="fluid-uuid", amount="1750")
   bundie_strategy_deposit("0xUSER", protocolId="moonwell-uuid", amount="1250")
...
1. Check balance: bundie_portfolio("0xBOT")
2. If idle USDC > 1000:
   bundie_get_recommendation("0xBOT", tokens=["USDC"], minRiskScore=80)
3. Deposit excess: bundie_deposit("0xBOT", "USDC", excessAmount)
...
```
1. bundie_set_preferences("0xUSER", minRiskScore=80, maxAllocationPerProtocol=25)
2. bundie_get_recommendation("0xUSER") → Deploy funds
3. Periodically:
   - bundie_portfolio("0xUSER") → Check positions
   - bundie_rebalance("0xUSER") → Compare vs optimal
...
Your Agent / Application
    ...
    ├─ Write: bundie_deposit, bundie_withdraw, bundie_strategy_*
    ├─ AI:    bundie_analyze_wallet, bundie_get_recommendation
    └─ State: bundie_set_preferences
         │
         ▼
